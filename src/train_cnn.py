...
def build_cnn(data: PreparedData, embedding_dim: int, num_filters: int, filter_sizes, dropout_rate: float = 0.1) -> Model:
...
def train_cnn(data: Optional[PreparedData] = None, project_root: Optional[Path] = None, epochs: int = 2,
              batch_size: int = 64, embedding_dim: int = DEFAULT_EMBEDDING_DIM,
              num_filters: int = DEFAULT_NUM_FILTERS, filter_sizes=None) -> Tuple[PreparedData, Model, dict]:
    if data is None:
    ...
    config = {"embedding_dim": embedding_dim, "num_filters": num_filters, "filter_sizes": filter_sizes}
